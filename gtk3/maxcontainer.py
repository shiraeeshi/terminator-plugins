...
class MaximizeContainerPlugin(plugin.Plugin):
    ...
    def do_select_container(self, terminal):
        ...
        def draw_first_terminal(widget, context):
            draw_as_selected(parents[-1])
            return(False)
        redraw_handler_id = terminal.vte.connect_after('draw', draw_first_terminal)
        def keypress_handler(widget, event):
            ...
            # in the receivers.
            if terminal.terminator.ibus_running:
		    if (event.state | Gdk.MODIFIER_MASK ) ^ Gdk.MODIFIER_MASK != 0:
    ...
    def callback(self, menuitems, menu, terminal):
        ...
        if self.is_maximized:
            item = Gtk.ImageMenuItem.new_with_mnemonic(_('Unmaximize container'))
            item.set_sensitive(not terminal.is_zoomed())
            item.connect('activate', lambda x: self.unmaximize(terminal))
            menuitems.append(item)
            return
        item = Gtk.ImageMenuItem.new_with_mnemonic(_('Maximize container...'))
